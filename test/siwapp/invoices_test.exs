defmodule Siwapp.InvoicesTest do
  ...
  describe "Statistics.get_amount_per_day/1" do
    ...
    test "for multiple invoices in the same day, it returns one unique value with the sum of its amounts",
      ...
      assert Enum.member?(Invoices.Statistics.get_amount_per_day(), {today, 1000})
    end
  end

  describe "Statistics.get_amount_per_currencies/1" do
    test "when there is no invoices it returns an empty map" do
      assert Invoices.Statistics.get_amount_per_currencies() == %{}
    end
    ...
    test "for multiple invoices with different currencies, there is a key and an amount for each one" do
      ...
      assert Invoices.Statistics.get_amount_per_currencies() == %{"USD" => 500, "GBP" => 500}
    end
    ...
    test "for multiple invoices with the same currency, there is only one key and accumulated amount for them" do
      ...
      assert Invoices.Statistics.get_amount_per_currencies() == %{"USD" => 1000}
    end
  end
end
