...
BEGIN
    ...
    WITH data(index, item) AS (SELECT * FROM items_one_recurring_invoice) SELECT json_object_agg(index-1,item) INTO result FROM data;
    return result;
END; $$;
...
SELECT json_agg(t) FROM (SELECT taxes, discount, quantity, description, unitary_cost FROM items_demo_extended WHERE common_id=1) t;
...
INSERT INTO invoices_id_conversion SELECT invoices.id, commons.id FROM invoices INNER JOIN commons ON invoices.name = commons.name AND invoices.identification = commons.identification ORDER BY invoices.id;

--items
INSERT INTO items(quantity, discount, description, unitary_cost, invoice_id) SELECT quantity, discount, description, unitary_cost, new_invoice_id(common_id) FROM items_demo WHERE is_invoice(common_id)=true;

-- payments
INSERT INTO payments("date", amount, notes, invoice_id, inserted_at, updated_at) SELECT "date", amount, notes, new_invoice_id(invoice_id), created_at, updated_at FROM payments_demo;
...
INSERT INTO items_id_conversion SELECT items.id, items_demo.id FROM items INNER JOIN items_demo ON items.description = items_demo.description ORDER BY items.id;
